    ...
    class EslClient : BaseThreadWorker, IEslClient
    {
        ...
        protected override void Worker()
        {
            ...
            while (!IsEx)
            {
                ...
                do
                {
                    int rclen = _sock.Receive(buffRx);
                    buffString = ASCIIEncoding.ASCII.GetString(buffRx, 0, rclen);
                    sb.Append(buffString);
                } while (_sock.Available > 0 || buffString.LastIndexOf("}"[0]) != buffString.Length - 1);
                ...
                foreach (string str in buffStringSplit)
                {
                        ...
                        && StreamEventContentLen > 0)
                    {
                        ...
                        {
                            _LogDelegate($"Event is malformed!", EslClientLogType.Error);
                            _LogDelegate($"Try to increase EslRxBufferSize and check metwork latency and the server performance", EslClientLogType.Error);
                            _LogDelegate($"Event received: {str}", EslClientLogType.Error);
                            _LogDelegate(str, EslClientLogType.Error);
                        }
            ...
            void SayByeBye()
            {
                ...
                */
                ...
                {
                    int rclen = _sock.Receive(buffRx, contentLen, SocketFlags.None);
        ...
        private void ObtainHostPort(out string host, out int port)
        {
            host = null;
            port = -1;
