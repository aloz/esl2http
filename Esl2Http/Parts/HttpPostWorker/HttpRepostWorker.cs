...
{
    class HttpRepostWorker : BaseThreadWorker, IHttpPostWorker
    {
        ...
        protected override void Worker()
        {
            using (HttpClient httpcli = new HttpClient())
            {
                ...
                while (!IsEx)
                {
                    try
                    {
                        string[] urls = _dal.GetHttpHandlersToRepost();
                        if (urls.Length > 0)
                        {
                            Parallel.ForEach(urls, url =>
                            {
                                List<Tuple<long, string>> EventsToPost = _dal.GetEventsToRepost(url);
                                    ...
                                    try
                                    {
                                        using (HttpRequestMessage req = new HttpRequestMessage(HttpMethod.Post, url))
                                        {
                                            ...
                                            */
                                            ...
                                            req.Headers.Add($"{Const.CONST_HTTP_HEADER_STARTSWITH}{Const.CONST_HTTP_HEADER_THIS_IS_RESEND}", "1");
