...
- _Update: if we successfully read JSON properties of the event - the event is valid by default, so if we read that it was `HEARTBEAT` to insert into `events_heartbeat_last` - the event is valid by default, and need no `column events_heartbeat_last.is_valid. `Right ideas come later` - a proverb _
