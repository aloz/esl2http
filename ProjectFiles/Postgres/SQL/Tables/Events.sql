CREATE TABLE events (
	...
	isvalid bool NOT NULL GENERATED ALWAYS AS ((event_jsonb ->> 'Core-UUID'::text) IS NOT NULL AND (event_jsonb ->> 'Event-Name'::text) IS NOT NULL AND (event_jsonb ->> 'Event-Date-Timestamp'::text) IS NOT NULL) STORED,
...
CREATE INDEX events_isvalid_idx ON events USING btree (isvalid);
